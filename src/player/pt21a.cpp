...
void PT21A_Player::mt_PlayVoice(int pat, int chn)
{
    uint32_t event = mdata.read32b(1084 + pat * 1024 + mt_PatternPos * 16 + chn * 4);
    if (event == 0) {   // TST.L   (A6)
        mt_PerNop(chn);
        return;
    }
...
void PT21A_Player::mt_SetPeriod(int chn)
{
    auto ch = mt_chantemp[chn];
    int note = ch.n_note & 0xfff;
...
void PT21A_Player::mt_CheckEfx(int chn)
{
    int cmd = mt_chantemp[chn].n_cmd & 0x0f;

    mt_UpdateFunk(chn);

    if (cmd == 0 && mt_chantemp[chn].n_cmdlo == 0) {
        mt_PerNop(chn);
        return;
    }
...
void PT21A_Player::mt_Arpeggio(int chn)
{
    auto ch = mt_chantemp[chn];
    ...
    // Arpeggio3
    int ofs = 37 * ch.n_finetune;  // MOVE.B  n_finetune(A6),D1 / MULU    #36*2,D1
...
void PT21A_Player::mt_PortaUp(int chn)
{
    auto ch = mt_chantemp[chn];
...
void PT21A_Player::mt_PortaDown(int chn)
{
    auto ch = mt_chantemp[chn];
    ch.n_period += ch.n_cmdlo & mt_LowMask;
    mt_LowMask = 0xff;
...
void PT21A_Player::mt_SetTonePorta(int chn)
{
    auto ch = mt_chantemp[chn];
    int note = ch.n_note & 0xfff;
    int ofs = 37 * ch.n_finetune;  // MOVE.B  n_finetune(A6),D0 / MULU    #37*2,D0
...
void PT21A_Player::mt_TonePortNoChange(int chn)
{
    auto ch = mt_chantemp[chn];
...
void PT21A_Player::mt_Vibrato2(int chn)
{
    auto ch = mt_chantemp[chn];
...
void PT21A_Player::mt_TonePlusVolSlide(int chn)
{
    mt_TonePortNoChange(chn);
    mt_VolumeSlide(chn);
}

void PT21A_Player::mt_VibratoPlusVolSlide(int chn)
{
    mt_Vibrato2(chn);
    mt_VolumeSlide(chn);
}
...
{
    int val = mt_chantemp[chn].n_cmdlo >> 4;
    if (val == 0) {
        mt_VolSlideDown(chn);
    } else {
        mt_VolSlideUp(chn, val);
    }
}

void PT21A_Player::mt_VolSlideUp(int chn, int val)
{
    auto ch = mt_chantemp[chn];
    ch.n_volume += val;
    if (ch.n_volume > 0x40) {
...
void PT21A_Player::mt_VolumeChange(int chn)
{
    auto ch = mt_chantemp[chn];
    if (ch.n_cmdlo > 0x40) {
        ch.n_cmdlo = 0x40;
...
{
    auto ch = mt_chantemp[chn];
    int row = (ch.n_cmdlo >> 4) * 10 + (ch.n_cmdlo & 0x0f);
    if (row <= 63) {
        // mt_pj2
        mt_PBreakPos = row;
    }
...
void PT21A_Player::mt_CheckMoreEfx(int chn)
{
    mt_UpdateFunk(chn);
...
void PT21A_Player::mt_E_Commands(int chn)
{
...
void PT21A_Player::mt_SetVibratoControl(int chn)
{
    auto ch = mt_chantemp[chn];
...
void PT21A_Player::mt_SetFinetune(int chn)
{
    auto ch = mt_chantemp[chn];
    ch.n_finetune = ch.n_cmdlo & 0x0f;
}

void PT21A_Player::mt_JumpLoop(int chn)
{
    auto ch = mt_chantemp[chn];
...
void PT21A_Player::mt_SetTremoloControl(int chn)
{
    auto ch = mt_chantemp[chn];
...
void PT21A_Player::mt_RetrigNote(int chn)
{
    auto ch = mt_chantemp[chn];
    int cmdlo = ch.n_cmdlo & 0x0f;
    if (cmdlo == 0) {
...
void PT21A_Player::mt_NoteCut(int chn)
{
    auto ch = mt_chantemp[chn];
...
void PT21A_Player::mt_FunkIt(int chn)
{
}

void PT21A_Player::mt_UpdateFunk(int chn)
{
}
