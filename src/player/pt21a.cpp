...
extern const uint8_t  mt_FunkTable[16];
extern const uint8_t  mt_VibratoTable[32];
extern const uint16_t mt_PeriodTable[16 * 37];
...
void PT21A_Player::mt_PlayVoice(const int pat, const int chn)
{
    ...
    if (ins > 0 && ins <= 31) {       // sanity check: was: ins != 0
        ...
        if (repeat) {                             // TST.W   D3 / BEQ.S   mt_NoLoop
            ...
            paula_->set_volume(chn, ch.n_volume);       // MOVE.W  D0,8(A5)        ; Set volume
        } else {
            // mt_NoLoop
...
void PT21A_Player::mt_CheckEfx(const int chn)
{
    ...
    default:
        // SetBack
        paula_->set_period(chn, ch.n_period);  // MOVE.W  n_period(A6),6(A5)
        switch (cmd) {
        case 0x7:
            mt_Tremolo(chn);
            break;
    ...
    //if (cmd != 0x7) {
    //    paula_->set_volume(chn, ch.n_volume << 2);
    //}
}
...
void PT21A_Player::mt_PerNop(const int chn)
{
    ...
    paula_->set_period(chn, period);  // MOVE.W  n_period(A6),6(A5)
}
...
void PT21A_Player::mt_Arpeggio(const int chn)
{
	...
	if (ch.n_period >= mt_PeriodTable[ofs + i]) {
	   // Arpeggio4
	   paula_->set_period(chn, mt_PeriodTable[ofs + i + val]);  // MOVE.W  D2,6(A5)
	   return;
	}
    }
}
...
void PT21A_Player::mt_PortaDown(const int chn)
{
    ...
    paula_->set_period(chn, ch.n_period);  // MOVE.W  D0,6(A5)
}
...
void PT21A_Player::mt_Tremolo(const int chn)
{
    ...
    // mt_TremoloOk
    paula_->set_volume(chn, volume);  // MOVE.W  D0,8(A5)
...
void PT21A_Player::mt_VolSlideUp(const int chn)
{
    ...
    paula_->set_volume(chn, ch.n_volume);
}
...
void PT21A_Player::mt_VolumeChange(const int chn)
{
    ...
    paula_->set_volume(chn, ch.n_volume);  // MOVE.W  D0,8(A5)
}
...
{
    auto& ch = mt_chantemp[chn];
    paula_->enable_filter((ch.n_cmdlo & 0x0f) != 0);
}
...
void PT21A_Player::mt_NoteCut(const int chn)
{
    ...
    ch.n_volume = 0;
    paula_->set_volume(chn, 0);  // MOVE.W  #0,8(A5)
}
...
{
    if (mt_counter) {    // TST.B   mt_counter
        return;
    }
    auto& ch = mt_chantemp[chn];
    const uint8_t val = (ch.n_cmdlo & 0x0f) << 4;
    ch.n_glissfunk = (ch.n_glissfunk & 0x0f) | val;
    if (val) {
        mt_UpdateFunk(chn);
    }
}
...
{
    auto& ch = mt_chantemp[chn];
    int idx = ch.n_glissfunk >> 4;
    if (idx == 0) {
        return;
    }
    ch.n_funkoffset += mt_FunkTable[idx];
    if (~ch.n_funkoffset & 0x80) {
        return;
    }
    ch.n_funkoffset = 0;

    const uint32_t loop_end = ch.n_loopstart + ch.n_replen * 2;
    ch.n_wavestart++;
    if (loop_end >= ch.n_wavestart) {
        ch.n_loopstart = ch.n_wavestart;
    }
    // mt_funkok
    mdata.write8(ch.n_wavestart, -1 - mdata.read8i(ch.n_wavestart));
}

namespace {

const uint8_t mt_FunkTable[16] = {
      0,   5,   6,   7,   8,  10,  11,  13,
